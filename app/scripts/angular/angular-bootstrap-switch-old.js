...
angular
		.module('frapontillo.bootstrap-switch')
		.directive(
				'bsSwitch',
				function ($timeout) {
return {
						...
						link : function link(scope, element, attrs, controller) {
							...
							/**
							 * *tweak* *
							 * Return the false value for this specific checkbox.
							 * 
							 * @returns {Object} representing the false view
							 *          value; if undefined, returns false.
							 */
							var getFalseValue = function() {
								var falseValue = attrs.ngFalseValue;
							...
							var listenToModel = function() {
											...
											if (newValue !== undefined) {
												$timeout(function() {
													element
															.bootstrapSwitch(
																	'state',
																	(newValue === getTrueValue()),
																	true);
												});
											}
										});
								scope.$watch('switchActive',
										function(newValue) {
											var active = newValue === true
													|| newValue === 'true'
													|| !newValue;
								...
								scope.$watch('switchOffColor', function(
										newValue) {
									attrs.dataOff = newValue;
									element.bootstrapSwitch('offColor',
											getValueOrUndefined(newValue));
								});
								scope.$watch('switchAnimate',
										function(newValue) {
											element.bootstrapSwitch('animate',
													scope.$eval(newValue
															|| 'true'));
										});
								...
								scope.$watch('switchLabel', function(newValue) {
									element.bootstrapSwitch('labelText',
											newValue ? newValue : '&nbsp;');
								});
								...
								scope.$watch('switchRadioOff', function(
										newValue) {
									element.bootstrapSwitch('radioAllOff',
											newValue === true
													|| newValue === 'true');
								});
							};
							...
							$timeout(function() {
								...
								// if model value is none among the defined values than dont do any any thing *tweak*
								if(controller.$modelValue && controller.$modelValue !== getTrueValue() && controller.$modelValue !== getFalseValue())
									return;
